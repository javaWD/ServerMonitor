...
import cn.com.esrichina.ServerMonitor.domain.Rights;

/**
 * Home object for domain model class Rights.
 * @see cn.com.esrichina.ServerMonitor.dao.Rights
...
public class RightsHome {
	...
	public Rights merge(Rights detachedInstance) {
		...
		try {
			Rights result = (Rights) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Rights findById(long id) {
		...
		try {
			Rights instance = (Rights) sessionFactory.getCurrentSession().get(
					"cn.com.esrichina.ServerMonitor.dao.Rights", id);
			if (instance == null) {
	...
	public List findByExample(Rights instance) {
		...
		try {
			List results = sessionFactory
					.getCurrentSession()
					.createCriteria("cn.com.esrichina.ServerMonitor.dao.Rights")
